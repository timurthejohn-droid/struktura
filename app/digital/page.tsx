...
const stages = [
  {
    ...
    actions: [
      { do: "Подключаем инженерную экспертизу на ранней стадии", value: "Чтобы архитектурная концепция сразу учитывала реальные производственные ограничения." },
      { do: "Выполняем предварительный BIM-анализ сложных зон", value: "Чтобы выявить технические риски до начала проектирования." },
      { do: "Оцениваем реализуемость, бюджет и сроки", value: "Чтобы избежать пересмотра стоимости и сроков после старта проекта." },
      { do: "Формируем техническую стратегию проекта", value: "Чтобы все участники двигались по единой инженерной логике." },
    ],
  },
  {
    slug: "rnd",
    ...
    actions: [
      { do: "Разрабатываем инженерную логику будущего решения", value: "Чтобы исключить конфликт между архитектурой, конструкцией и производством." },
      { do: "Выполняем точные цифровые расчёты", value: "Чтобы заранее определить объёмы материалов и избежать перерасхода." },
      { do: "Интегрируем подсистемы в единую цифровую модель", value: "Чтобы исключить коллизии между разделами проекта." },
      { do: "Проверяем решения до передачи в производство", value: "Чтобы ошибки не переходили на следующие этапы." },
    ],
  },
  {
    slug: "design",
    ...
    actions: [
      { do: "Проектируем все элементы в единой BIM-среде", value: "Чтобы все разделы работали по одной цифровой модели." },
      { do: "Разрабатываем нестандартные узлы и крепления", value: "Чтобы сложная архитектура оставалась реализуемой." },
      { do: "Выпускаем детализированную рабочую документацию", value: "Чтобы производство получало точные данные без дополнительных уточнений." },
      { do: "Подготавливаем модель к переходу в CAM", value: "Чтобы производство начиналось без ручной переработки модели." },
    ],
  },
  {
    slug: "production",
    ...
    actions: [
      { do: "Передаём данные напрямую из цифровой модели (BIM2CAM)", value: "Чтобы исключить ошибки при передаче информации в производство." },
      { do: "Организуем производство под специфику проекта", value: "Чтобы проект не ограничивался возможностями одного производства." },
      { do: "Контролируем качество на каждом этапе", value: "Чтобы все элементы соответствовали проекту ещё до монтажа." },
      { do: "Выполняем предсборку и инженерную проверку", value: "Чтобы исключить переделки на строительной площадке." },
    ],
  },
  {
    slug: "logistics",
    ...
    actions: [
      { do: "Планируем поставки по графику монтажа", value: "Чтобы материалы поступали точно в нужный момент." },
      { do: "Контролируем упаковку и транспортировку", value: "Чтобы сохранить геометрию и качество элементов." },
      { do: "Координируем поставки с генподрядчиком", value: "Чтобы исключить простои монтажных бригад." },
      { do: "Используем систему маркировки и цифрового контроля", value: "Чтобы каждый элемент быстро находился и устанавливался на своё место." },
    ],
  },
  {
    slug: "assembly",
    ...
    actions: [
      { do: "Выполняем шеф-монтаж и авторский контроль", value: "Чтобы реализация полностью соответствовала проекту." },
      { do: "Проверяем геометрию перед установкой", value: "Чтобы избежать накопления ошибок при сборке." },
      { do: "Работаем по единому проектному графику", value: "Чтобы синхронизировать монтаж со всеми участниками проекта." },
      { do: "Оперативно вносим инженерные корректировки", value: "Чтобы изменения на объекте не приводили к срыву сроков." },
    ],
  },
];

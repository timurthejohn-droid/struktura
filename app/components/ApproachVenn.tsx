 ...
 *      в центре появляется STRUKTURA+.
 *   5. Схема гаснет, остаётся STRUKTURA+, затем гаснет и логотип —
 *      после этого прокрутка открывает блок четырёх принципов.
 */
...
const CIRCLES: { dir: [number, number]; label: string; lx: number; ly: number }[] = [
  { dir: [0, -1], label: "Детерминированность", lx: 500, ly: 238 },
  { dir: [1, 0], label: "Конечность", lx: 728, ly: 506 },
  { dir: [0, 1], label: "Массовость", lx: 500, ly: 776 },
  { dir: [-1, 0], label: "Дискретность", lx: 272, ly: 506 },
];
...
export default function ApproachVenn() {
  ...
  // Финальный выход: сначала гасим саму схему, оставляя только логотип.
  const diagramOpacity = useTransform(scrollYProgress, [0.9, 0.95], [1, 0]);
  const headingOpacity = useTransform(scrollYProgress, [0, 0.08, 0.88, 0.94], [0, 1, 1, 0]);
  ...
  const logoOpacity = useTransform(scrollYProgress, [0.9, 0.945, 0.985, 1], [0, 1, 1, 0]);

  if (reduced) return <StaticVenn />;

  return (
    <section ref={wrapRef} style={{ height: "560vh", background: "var(--paper)", borderTop: "1px solid var(--line-light)" }}>
      <div className="sticky top-0 z-10 h-screen flex items-center justify-center overflow-hidden">
        <motion.div className="absolute top-24 left-0 right-0 pointer-events-none" style={{ opacity: headingOpacity }}>
        ...
        <motion.svg
          ...
          style={{ width: "min(92vw, 86vh)", height: "min(92vw, 86vh)", scale, opacity: diagramOpacity }}
